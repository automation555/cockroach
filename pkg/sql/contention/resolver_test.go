...
func TestResolver(t *testing.T) {
	...
	t.Run("retry_after_encountering_provisional_value", func(t *testing.T) {
		...
		expected[0].BlockingEvent.TxnMeta.ID = activeTxnID
		expected[0].BlockingEvent.TxnMeta.CoordinatorNodeID = 1
		expected[0].BlockingTxnFingerprintID = 1000
		...
		input = input[:1]
		input[0].BlockingEvent.TxnMeta.ID = newTxn.TxnID
		input[0].BlockingEvent.TxnMeta.CoordinatorNodeID = newTxn.coordinatorNodeID
		expected[1].BlockingEvent.TxnMeta.ID = newTxn.TxnID
		expected[1].BlockingEvent.TxnMeta.CoordinatorNodeID = newTxn.coordinatorNodeID
		expected[1].BlockingTxnFingerprintID = newTxn.TxnFingerprintID
...
func generateUnresolvedContentionEventsFromTestCases(
	...
	for _, tc := range tcs {
		...
		event.BlockingEvent.TxnMeta.CoordinatorNodeID = tc.coordinatorNodeID
