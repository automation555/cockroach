		...
		for _, event := range currentBatch {
			if txnFingerprintID, ok := resolvedTxnIDs[event.BlockingEvent.TxnMeta.ID]; ok {
				...
				if txnFingerprintID == roachpb.InvalidTransactionFingerprintID {
					...
					delete(s.mu.retryRecord, event.BlockingEvent.TxnMeta.ID)
					continue
				}
...
func (s *resolverQueueImpl) maybeRequeueEventForRetryLocked(
	...
	// event.
	remainingRetryBudget, ok := s.mu.retryRecord[event.BlockingEvent.TxnMeta.ID]
	if !ok {
		s.mu.unresolvedEvents = append(s.mu.unresolvedEvents, event)
		s.mu.retryRecord[event.BlockingEvent.TxnMeta.ID] = defaultBudget
	...
	s.mu.retryRecord[event.BlockingEvent.TxnMeta.ID]--
	s.mu.unresolvedEvents = append(s.mu.unresolvedEvents, event)
	return true /* requeued */
}
...
func makeRPCRequestFromBatch(
	...
	req := &serverpb.TxnIDResolutionRequest{
		CoordinatorNodeID: batch[0].BlockingEvent.TxnMeta.CoordinatorNodeID,
