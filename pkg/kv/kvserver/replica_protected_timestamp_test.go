...
func TestCheckProtectedTimestampsForGC(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			mc := &manualCache{}
			tsc.ProtectedTimestampReader = mc
...
func (c *manualCache) GetEarliestValidProtectionTimestamp(
	ctx context.Context, sp roachpb.Span, threshold hlc.Timestamp,
) (protectionTimestamp hlc.Timestamp, asOf hlc.Timestamp) {
	readAt := c.Iterate(ctx,
		sp.Key,
		sp.EndKey,
		func(rec *ptpb.Record) (wantMore bool) {
			protectionTimestamp = rec.Timestamp
			return false
		})
	if isValid := threshold.LessEq(protectionTimestamp); !isValid {
		return hlc.Timestamp{}, readAt
	}
