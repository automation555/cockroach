...
// the only remaining source of the data in the entries is state machine
// (which a prefix of the log), so if we ever truncate a log entry that is not
// durably on the state machine, we risk losing the information in said log
// entries which could lead to a scenario in which we violate Raft's leader
// completeness property: the replica may become leader and be asked for log
// entries it can no longer provide (neither directly nor through a snapshot).
