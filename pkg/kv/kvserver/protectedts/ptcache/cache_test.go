...
func TestGetEarliestProtectionTimestampAboveThreshold(t *testing.T) {
	ctx := context.Background()
	tc := testcluster.StartTestCluster(t, 1, base.TestClusterArgs{})
	defer tc.Stopper().Stop(ctx)
	// Set the poll interval to be very long.
	...
	for _, testCase := range []struct {
		...
		// Single span tests.
		{
			name: "basic",
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, cleanup func(...*ptpb.Record)) {
				r, _ := protect(t, s, p, ts(10), sp42)
				require.NoError(t, c.Refresh(ctx, s.Clock().Now()))

				protectionTS, _ := c.GetEarliestValidProtectionTimestamp(ctx, sp42, ts(5))
				require.Equal(t, ts(10), protectionTS)
				cleanup(r)
			},
		},
		{
			...
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, cleanup func(...*ptpb.Record)) {
				...
				protectionTS, _ := c.GetEarliestValidProtectionTimestamp(ctx, sp42, ts(5))
				require.Equal(t, ts(6), protectionTS)
				cleanup(r1, r2, r3)
			},
		},
		{
		...
		{
			name: "record at threshold",
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, cleanup func(...*ptpb.Record)) {
				r1, _ := protect(t, s, p, ts(10), sp42)
				r2, _ := protect(t, s, p, ts(5), sp42)
				require.NoError(t, c.Refresh(ctx, s.Clock().Now()))

				protectionTS, _ := c.GetEarliestValidProtectionTimestamp(ctx, sp42, ts(5))
				require.Equal(t, ts(5), protectionTS)
				cleanup(r1, r2)
			},
		},
		{
			name: "no valid record",
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, cleanup func(...*ptpb.Record)) {
				r1, _ := protect(t, s, p, ts(10), sp42)
				r2, _ := protect(t, s, p, ts(5), sp42)
				require.NoError(t, c.Refresh(ctx, s.Clock().Now()))

				protectionTS, _ := c.GetEarliestValidProtectionTimestamp(ctx, sp42, ts(20))
				require.Equal(t, hlc.Timestamp{}, protectionTS)
				cleanup(r1, r2)
			},
		},
		{
			name: "no records",
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, _ func(...*ptpb.Record)) {
				require.NoError(t, c.Refresh(ctx, s.Clock().Now()))
				protectionTS, _ := c.GetEarliestValidProtectionTimestamp(ctx, sp42, ts(20))
				require.Equal(t, hlc.Timestamp{}, protectionTS)
			},
		},
		// Check protection timestamps over a keyspan that is stored as multiple
		// spans with different protection timestamp records underneath the hood.
		{
			name: "multiple spans",
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, cleanup func(...*ptpb.Record)) {
				r1, _ := protect(t, s, p, ts(10), sp42)
				r2, _ := protect(t, s, p, ts(5), sp43)
				require.NoError(t, c.Refresh(ctx, s.Clock().Now()))

				protectionTS, _ := c.GetEarliestValidProtectionTimestamp(ctx, sp4243, ts(4))
				require.Equal(t, ts(5), protectionTS)
				cleanup(r1, r2)
			},
		},
		{
			name: "multiple spans expired",
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, cleanup func(...*ptpb.Record)) {
				r1, _ := protect(t, s, p, ts(10), sp42)
				r2, _ := protect(t, s, p, ts(5), sp43)
				require.NoError(t, c.Refresh(ctx, s.Clock().Now()))

				protectionTS, _ := c.GetEarliestValidProtectionTimestamp(ctx, sp4243, ts(6))
				require.Equal(t, ts(10), protectionTS)
				cleanup(r1, r2)
			},
		},
		{
			name: "multiple spans multiple records",
			test: func(t *testing.T, p protectedts.Storage, c *ptcache.Cache, cleanup func(...*ptpb.Record)) {
				r1, _ := protect(t, s, p, ts(10), sp42)
