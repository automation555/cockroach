...
// - hasOverlap is true if there is an overlap between the source and the
// destination values such that the destination is in the middle of the source.
// If the caller is unsure, then they should pass in true.
func (b *Bytes) ensureLengthForAppend(src *Bytes, destIdx, newLen int, hasOverlap bool) {
	...
	//   b = {a a a}.
	mustReallocate := cap(b.elements) < newLen || (b == src && hasOverlap)
	if mustReallocate {
...
func (b *Bytes) AppendSlice(src *Bytes, destIdx, srcStartIdx, srcEndIdx int) {
	...
	b.ensureLengthForAppend(src, destIdx, newLen, srcEndIdx > destIdx /* hasOverlap */)
	...
	for i := 0; i < len(srcElementsToCopy); i++ {
		if e := &b.elements[destIdx+i]; e.nonInlined {
...
func (b *Bytes) appendSliceWithSel(src *Bytes, destIdx int, sel []int) {
	...
	// Get a handle on the source elements before calling ensureLengthForAppend
	// so that in case when b == src we have the uncorrupted source slice.
	srcElements := src.elements
	newLen := destIdx + len(sel)
	// We're not sure whether there is an overlap, so we pass in true, to be
	// safe.
	b.ensureLengthForAppend(src, destIdx, newLen, true /* hasOverlap */)
	for i, srcIdx := range sel {
		b.elements[destIdx+i].copy(srcElements[srcIdx], b, src)
	}
}
